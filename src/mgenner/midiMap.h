...
class midiMap {
   public:
    ...
    //获取文本
    inline void
    getDescription(const stringPool::stringPtr& key, int tm, const std::function<void(const std::string&)>& callback) {
    ...
    inline void
    findDescription(int tm_begin, int tm_end, const std::function<void(int, const std::string&)>& callback) {
